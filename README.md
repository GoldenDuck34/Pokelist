# Pokelist
A Pokédex tracker for every mainline Pokémon game. Simply and easy to use.

# Controls:
- Right click to shiny a Pokémon. Each Pokémon has a separate tracker for shinies.
- You can mark favorites, mark off the circles, and perform other obvious actions.
- Empty circles next to Pokémon in the top right are mega stones. Left click to mark off the mega stone, right click to show the mega image of the Pokémon.

# Credits:
- All code by me
- The images by pokeapi up to ultra sun and ultra moon
- The images by wikidex after that
- ChatGPT for the Pokéball icon
- Serebii for the mega stone images
